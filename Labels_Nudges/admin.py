...
class Personal_infoAdmin(admin.ModelAdmin):
    list_display = ('id','session_id','created','age','gender','country','education','FK_9','FK_10','FK_11','FK_12')
...
class RecommedationsAdmin(admin.ModelAdmin):
    list_display = ('id','person','recommended_recipes', 'created')
    actions = [export_as_csv_action("CSV Export")]
...
class SelectedContentAdmin(admin.ModelAdmin):
    list_display =  ('id','person','contentIDs')
    action = [export_as_csv_action("CSV EXPORT")]
    
admin.site.register(SelectedContent, SelectedContentAdmin)
